...
class ScreenHandle(pygame.Surface):
    ...
    def connect_engine(self, game_engine):
        if self.successor is not None:
            self.game_engine = game_engine
            self.successor.connect_engine(game_engine)
...
class GameSurface(ScreenHandle):
    ...
    def draw_map(self):
        ...
        min_x, min_y = self.calculate_min(self.game_engine.sprite_size, self.game_engine.hero.position)
        ##
    ...
    def draw_object(self, sprite, coord):
        ...
        min_x, min_y = self.calculate_min(size, self.game_engine.hero.position)
    ...
    @staticmethod
    def calculate_min(size, position):
        min_x = position[0] - (300 // size)
        if min_x < 0:
            min_x = 0
        min_y = position[1] - (200 // size)
        if min_y < 0:
            min_y = 0
        return min_x, min_y
    ...
    def draw(self, canvas):
        ...
        super(GameSurface, self).draw(canvas)
...
class ProgressBar(ScreenHandle):
    ...
    def connect_engine(self, engine):
        self.game_engine = engine
        super(ProgressBar, self).connect_engine(engine)
        # FIXME save engine and send it to next in chain
...
class InfoWindow(ScreenHandle):
    ...
    def connect_engine(self, engine):
        ...
        super(InfoWindow, self).connect_engine(engine)
...
class HelpWindow(ScreenHandle):
    ...
    def connect_engine(self, engine):
        self.game_engine = engine
        super(HelpWindow, self).connect_engine(engine)
        # FIXME save engine and send it to next in chain
