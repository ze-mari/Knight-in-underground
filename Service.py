...
class SpecialMap(MapFactory):
            ...
            for i in range(41):
                for j in range(41):
                    if i == 0 or j == 0 or i == 40 or j == 40:
                        ...
                        self.Map[j][i] = [wall, floor1, floor2, floor3, floor3,
                                          floor2, floor3, floor1][random.randint(0, 7)]
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for obj_name in object_list_prob['ally']:
                prop = object_list_prob['ally'][obj_name]
                for i in range(random.randint(prop['max-count'], prop['max-count'] + 2)):
                    coord = (random.randint(1, 39), random.randint(1, 39))
                    intersect = True
                    while intersect:
                        intersect = False
            ...
            for obj_name in object_list_prob['enemies']:
                if obj_name not in self.config.keys():
                    continue
                prop = object_list_prob['enemies'][obj_name]
